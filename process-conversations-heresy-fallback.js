...
// Palavras-chave para detecção de heresia
const HERESY_KEYWORDS = {
  ...
  doctrinal_heresy: [
    'salvação por obras', 'maria é deusa', 'papa é deus',
    'reencarnação cristã', 'múltiplos deuses', 'jesus não é deus',
    'trindade é falsa', 'batismo salva', 'santos salvam'
  ],
  false_teaching: [
    'teologia da prosperidade extrema', 'determinismo total',
    'universalismo', 'gnosticismo', 'arianismo', 'modalismo'
  ],
  inappropriate: [
    'sexo explícito', 'pornografia', 'drogas ilegais',
    'violência extrema', 'suicídio incentivo', 'ódio racial'
  ]
};

// Função para detectar heresia por palavras-chave
function detectHeresyByKeywords(message) {
  ...
  return { isHeresy: false, category: 'appropriate', confidence: 90 };
}

// Função para classificar mensagem usando IA (com fallback)
...
RAZÃO: [breve explicação]`;
  ...
  try {
    ...
    if (!response.ok) {
      console.log(`⚠️ API Gemini falhou (${response.status}), usando análise por palavras-chave...`);
      
      // Fallback para análise por palavras-chave
      const keywordResult = detectHeresyByKeywords(message);
      return {
        classification: keywordResult.isHeresy ? keywordResult.category : "appropriate_content",
        confidence: keywordResult.confidence,
        reason: keywordResult.isHeresy ? `Palavra-chave detectada: ${keywordResult.keyword}` : "Análise por palavras-chave - conteúdo apropriado",
        isHeresy: keywordResult.isHeresy,
    ...
    return { classification, confidence, reason, isHeresy, method: "ai" };
  } catch (error) {
    console.log("⚠️ Erro na API, usando análise por palavras-chave...");
...
async function processConversationsForHeresy() {
  try {
    ...
    if (apiError || !apiKeyData?.encrypted_key) {
      console.log('⚠️ Chave da API Gemini não encontrada, usando apenas análise por palavras-chave');
    } else {
      geminiApiKey = apiKeyData.encrypted_key;
    ...
    for (const conversation of conversationsToAnalyze) {
      ...
      // Classificar com IA ou palavras-chave
      const result = await classifyMessageWithAI(userMessage, geminiApiKey);
      
      console.log(`🤖 Classificação: ${result.classification} (${result.confidence}%) [${result.method}]`);
      console.log(`💭 Razão: ${result.reason}`);
      
      if (result.isHeresy && result.confidence >= 70) {
        ...
        const { data: newLog, error: logError } = await supabase
          ...
          .insert({
            ...
            action_taken: result.method === 'ai' ? 'ai_classified_heresy' : 'keyword_classified_heresy',
            ai_classification: `${result.classification}: ${result.reason}`,
